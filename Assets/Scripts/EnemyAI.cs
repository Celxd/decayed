...
public class EnemyAI : MonoBehaviour
{
    ...
    public GameObject projectilePrefab; // Reference to the enemy's projectile prefab
    public Transform projectileSpawnPoint; // The point from where the projectile is fired
    public float projectileSpeed = 10f; // The speed of the projectile
    public float fireRate = 1f; // The rate at which the enemy can shoot
    public float shootingRange = 10f; // The maximum range at which the enemy can shoot
    public LayerMask shootingMask; // The layer(s) on which the enemy can shoot
    ...
    void Start()
    {
        ...
        navMeshAgent.SetDestination(waypoints[m_CurrentWaypointIndex].position);    //  Set the destination to the first waypoint
                                                                                    // Find the player GameObject using a tag or another method
    ...
    private void Update()
    {
        ...
        // Check if the player is within range and line of sight
        if (distanceToPlayer <= shootingRange && Physics.Linecast(transform.position, player.position, shootingMask))
        {
            // Increment the time elapsed since the last shot
            timeSinceLastShot += Time.deltaTime;

            // Check if enough time has elapsed since the last shot to shoot again
            if (timeSinceLastShot >= 1f / fireRate)
            {
                // Reset the time elapsed since the last shot
                timeSinceLastShot = 0f;

                // Instantiate a new instance of the enemy's projectile prefab
                ...
                // Set the velocity of the projectile to fire towards the player
